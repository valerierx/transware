...
public class FileUtils {
    private static String minecraftDir = Minecraft.getMinecraft().mcDataDir.getPath();
    ...
    // Create a directory in the minecraft root directory
    public static File mkDir(String dir) {
        File folder = new File(minecraftDir, dir);
        if (!folder.exists())
            folder.mkdir();
        return folder;
    }
    // Create a json file in a specified directory
    public static File mkJSON(File dir, String name) {
    ...
    public static void saveJsonFile(File file, JsonObject jsonObject) {
        try {
            ...
            } catch (Throwable i_need_a_variable_name_fuck) {
                throwable = i_need_a_variable_name_fuck;
                throw i_need_a_variable_name_fuck;
            } finally {
                if (throwable != null) {
                    try {
                        writer.close();
                    } catch (Throwable i_still_cant_find_a_name) {
                        throwable.addSuppressed(i_still_cant_find_a_name);
                    }
                } else {
                    writer.close();
                }
